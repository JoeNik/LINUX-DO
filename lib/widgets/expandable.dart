...
class ExpandablePanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // 处理展开状态变化并触发回调
    void handleExpandedChange(bool expanded) {
      if (onExpanded != null) {
        onExpanded!(expanded);
      }
    }
        ...
        return wrap
            ? ExpandableButton(
                ...
                onPressed: () {
                  ...
                  if (controller != null) {
                    final wasExpanded = controller.expanded;
                    controller.toggle();
                    // 只在状态确实发生变化时触发回调
                    if (wasExpanded != controller.expanded) {
                      handleExpandedChange(controller.expanded);
                    }
                  }
                },
                child: widget,
    ...
    if (controller != null) {
      ...
      if (controller == null) {
        ...
        // 监听控制器状态变化
        if (onExpanded != null) {
          return _ExpandedChangeNotifier(
            controller: controller,
...
class ExpandableButton extends StatelessWidget {
  ...
  const ExpandableButton({super.key, this.child, this.theme, this.onPressed});
  ...
  Widget build(BuildContext context) {
    ...
    if (theme.useInkWell!) {
      return InkWell(
        onTap: onPressed ?? controller?.toggle,
...
/// 用于监听ExpandableController状态变化的StatefulWidget
class _ExpandedChangeNotifier extends StatefulWidget {
  final ExpandableController controller;
  final Function(bool) onExpanded;
  final Widget child;

  const _ExpandedChangeNotifier({
  ...
  @override
  _ExpandedChangeNotifierState createState() => _ExpandedChangeNotifierState();
}

class _ExpandedChangeNotifierState extends State<_ExpandedChangeNotifier> {
  ...
  void initState() {
    super.initState();
    _lastExpandedState = widget.controller.expanded;
    ...
    widget.controller.addListener(_handleExpandedChange);
  }
  
  @override
  void dispose() {
    widget.controller.removeListener(_handleExpandedChange);
    super.dispose();
  }
  
  void _handleExpandedChange() {
    // 只有在完成初始化后且状态确实发生变化时才触发回调
    if (_isInitialized && _lastExpandedState != widget.controller.expanded) {
      _lastExpandedState = widget.controller.expanded;
      widget.onExpanded(widget.controller.expanded);
    }
  }
